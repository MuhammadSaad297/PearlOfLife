...
export class MemoryFolderDetailsComponent implements OnInit {
  ...
  folderResult = [
    { date: '2025-04-17T09:47:17.326Z', url: 'https://cloudinary-marketing-res.cloudinary.com/images/w_1000,c_scale/v1679921049/Image_URL_header/Image_URL_header-png?_i=AA?text=Slide+1' },
    { date: '2025-08-17T09:47:17.326Z', url: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQcTY50AnR35-aKaONIPoeLNh_KrvAq9bwD7A&s?text=Slide+2' },
    { date: '2025-04-30T09:47:17.326Z', url: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTU3HFVnkYFJ_OIogo__Qv58bmhwRqZJcQhOA&s?text=Slide+3' }
  ];
  ...
  getFolderDetails() {
    const queryParams = objectToQueryParams({ ...this.pageOptions, folder_id: this.folder_id });
    this.memoriesService.getMemoriesFolderDetails(queryParams).subscribe({
      next: (response: any) => {
        ...
        this.folderDetails.forEach((item: any) => {
          item.image_details.image_path = this.baseUrl + item.image_details.image_path.slice(1);
        });
